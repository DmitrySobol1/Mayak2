...
async function getTheatreOrGenre() {
  const response = await fetch(
    `https://api.directual.com/good/api/v5/data/2_spectacleorplace/getInfoAboutCurrentSpectacleOrPlace?appID=5481b0b8-ec7f-457d-a582-3de87fb4f347&sessionID=&item_id=${choosedTheatreGenre}`,
    {
      method: 'GET',
      headers: {
  ...
  subcategory.addEventListener('click', async () => {
    ...
    try {
      ...
      array.forEach((e) => {
        ...
        btn.addEventListener('click', () => {
          // console.log(`Выбрана категория: ${e.subCategory_id.subcategory_name}`);
          //   ageBtn_content = document.getElementById('ageBtn_content');
